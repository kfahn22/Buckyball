// https://editor.p5js.org/kfahn/sketches/7LcbjlbSV
...
function setup() {
  ...
  for (let i = 0; i < cols * rows; i++) {
    ...
    do {
      ...
      //let overlap = random(0.5, 1.0);
      //let depth = int(map(overlap, 0.5, 1.0, 6, 2));
      // console.log(d)
      depth = int(random(2, 5));
      let a = map(overlap, 0.5, 1.0, 150, 225);
      palette = generatePaletteArray(url1, 220);
      config = `${n}-${overlap.toFixed(2)}-${depth}-${idx}`; // Create a unique string for this configuration
      ...
      if (!uniqueConfigs.has(config)) {
        ...
        // pyramid
        //buffer.translate(spriteSize / 2, spriteSize *2/3);
    ...
    } while (!pattern); // Repeat until a unique pattern is generated
  ...
  saveCanvas(spriteSheet, "spritesheet", "jpg");
}
